...
data <- data[, !colnames(data) %in% c("ï..X","Y","Index_","event_unique_id","ucr_code","ucr_ext","FID")]
...
Clustering on lat/long to look at natural crime hotspots vs compared with manual "clustering" aka neighbourhood divisions
...
data_for_gower = data[,c("occurrencetime","reportedtime","premisetype","MCI")]
data_for_gower$occurrencetime = as.numeric(data_for_gower$occurrencetime)
data_for_gower$reportedtime = as.numeric(data_for_gower$reportedtime)
# Are there skewed data?
gower_dist <- daisy(sample_n(data_for_gower,nrow(data_for_gower)*0.5),metric = "gower")
```
