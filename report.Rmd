...
We can see that there are 32 missing values, all in occurence date related columns. Upon further inspection, these are all the same rows. We can also see that the occurrence date value is correct, so these date type columns can have their missing values imputed. 
...
wssplot(hood, nc=15)
```
Now it seems number 3 is a good choice of number of clusters. We then proceed to perform K-means clustering on the data-set. We have followed this tutorial(https://www.datanovia.com/en/lessons/cluster-validation-statistics-must-know-methods/) to evaluate the quality of clustering results. Specifically, we have used Silhouette width as an quantitative measures to evaluate the clustering quality. As we will see in the Silhouette plot, both in cluster #2 and cluster #3 have data points where the Silhouette width falls to negative, indicating a not so ideal clustering. This can also be observed in the 1st plot where we see that cluster #2 and cluster #3 is quite close to each other.
```{r}
# K-means clustering
...
We can see that cluster 3 has lower than average crime. It has 93 neighbourhoods, meaning that the majority of Toronto is quite safe in terms of number of incidents occuring. Cluster 2 has slightly above average crime numbers, and it only has 37 neighbourhoods. 10 neighbourhoods have much higher than average crime numbers, which is seen in cluster 3. In some way, we can intepret the results as such that crime is concentrated in these small pockets of Toronto.

As an interesting activity, we also looking at a 3D representation of the clustering results. With 3 princeple components, more variations should be captured that as in the case in 2 princeple components. With a third dimension, we can see that there is more of a spread than initially can be seen simply in two dimensions.
```{r}
#3D plot
pc <-princomp(hood, cor=TRUE, scores=TRUE)
plot3d(pc$scores[,1:3], col=km.res$cluster, main="k-means clusters")
```
Next we can look at the neighbourhoods from a heirarchichal clustering approach. Again, we need to determine how many clusters we want to have. For heirarchichal clustering, we look at the total number of observations that end up in different clusters with different configuration(in this case, the configuration is the number of clusters).
```{r}
counts <- sapply(2:6, function(ncl) eclust(hood, "hclust", k = ncl, hc_metric = "euclidean", hc_method = "ward.D2")$size)
names(counts) <- 2:6
counts
```
Still, we can see that 3 clusters is not bad, if we consider only the cluster size as the criterier for choosing the number of clusters. We don't want to split the neighbourhoods into clusters with a small number of neighbourhoods.

Now that we have choose the number of clusters, we proceed to the clustering process. As previously in k-means, we look at Silhouette width as an quantitative measures to evaluate the clustering quality. To visually inspect the clusters, we plot dendrograms.
```{r}
...
As in kmeans, we determine the number of clusters based on the trends shown in within cluster sum of squares.
```{r}
library(rlist)
set.seed(1234)
